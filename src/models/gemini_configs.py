"""Gemini-based agent configurations that actually work."""

from .config import (
    AgentConfig, ModelConfig, AgentRole, ModelProvider
)


def get_gemini_agent_config(role: AgentRole) -> AgentConfig:
    """Get agent configuration using Gemini models."""
    
    # Different Gemini models for different purposes
    GEMINI_PRO = "gemini-1.5-pro"      # Best quality, higher cost
    GEMINI_FLASH = "gemini-1.5-flash"  # Fast and efficient
    GEMINI_FLASH_8B = "gemini-1.5-flash-8b"  # Fastest, lowest cost
    ...
    configs = {
        AgentRole.ORCHESTRATOR: AgentConfig(
            ...
            model_config=ModelConfig(
                ...
                model=GEMINI_FLASH,  # Fast for orchestration
                temperature=0.3,
                max_tokens=4096
            ),
        ...
        AgentRole.HOOK_SPECIALIST: AgentConfig(
            ...
            model_config=ModelConfig(
                provider=ModelProvider.GEMINI,
                model=GEMINI_PRO,  # Best quality for creative tasks
                temperature=0.8,
                max_tokens=2048
            ),
        ...
        AgentRole.STORY_ARCHITECT: AgentConfig(
            ...
            model_config=ModelConfig(
                provider=ModelProvider.GEMINI,
                model=GEMINI_PRO,  # Quality for narrative
                temperature=0.6,
                max_tokens=4096
            ),
        ...
        AgentRole.CTA_STRATEGIST: AgentConfig(
            ...
            model_config=ModelConfig(
                provider=ModelProvider.GEMINI,
                model=GEMINI_FLASH,  # Fast for structured output
                temperature=0.4,
                max_tokens=1024
            ),
        ...
        AgentRole.RESEARCH_ANALYST: AgentConfig(
            ...
            model_config=ModelConfig(
                provider=ModelProvider.GEMINI,
                model=GEMINI_PRO,  # Best for accuracy
                temperature=0.1,
                max_tokens=4096
            ),
        ...
        AgentRole.STYLIST: AgentConfig(
            ...
            model_config=ModelConfig(
                provider=ModelProvider.GEMINI,
                model=GEMINI_FLASH,  # Fast for style matching
                temperature=0.7,
                max_tokens=2048
            ),
        ...
        AgentRole.CHALLENGER: AgentConfig(
            ...
            model_config=ModelConfig(
                provider=ModelProvider.GEMINI,
                model=GEMINI_FLASH,  # Fast for analysis
                temperature=0.5,
                max_tokens=2048
            ),
